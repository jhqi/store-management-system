...
class update_out_log_window(QMainWindow, Ui_update_out_log):
    ...
    def on_both_btn_clicked(self):
        ...
        cur.close()
        conn.close()
        QMessageBox.question(self, '修改成功！', '出库记录及对应物料库存余量同时修改！', QMessageBox.Yes)
        self.close()
    ...
    def update_log(self):
        ...
        if self.comboBox.currentIndex() == 1:  # 还是出库
            ...
            conn.text_factory = str
            cur = conn.cursor()
            sql = "update out_log set date_time=" + log_date + ",out_num=" + num + ", per_price=" + per_price + ", pos='" + position + "', total_price=" + total_price + ", user_man='" + user_man + "', agree_man='" + agree_man + "' where out_log_id=" + self.ori_out_log_id
            cur.execute(sql)
            conn.commit()
            cur.close()
            conn.close()
            ...
            t_in_log = In_Log(log_date, id, name, spec, num, per_price, position, total_price, user_man, agree_man)
            t_in_log.new_in_log()
        return 1
